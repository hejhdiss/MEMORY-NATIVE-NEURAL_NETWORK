/**
 * NEURAL DIFFERENTIAL MANIFOLDS (NDM) WITH MOMENTUM
 * 
 * Advanced architecture with continuous weight evolution enhanced by momentum:
 * V(t+1) = β * V(t) + α * ∇W
 * W(t+1) = W(t) + V(t+1)
 * 
 * Key Innovation: Momentum-damped neuroplasticity for smoother weight evolution
 * 
 * Compile to DLL/SO:
...
typedef struct {
    ...
    // === CORE WEIGHTS (These evolve via ODEs with momentum) ===
    ...
    // === WEIGHT VELOCITIES (Momentum terms) ===
    ...
    // === TRAINING PARAMETERS ===
    float learning_rate;
    float momentum;           // Momentum coefficient (β)
    int training_steps;
    float last_loss;
...
EXPORT NDMNetwork* create_ndm(int input_size, int hidden_size, int output_size,
    ...
    net->momentum = 0.9f;           // Default momentum coefficient
    net->dt = 0.1f;
    ...
    // Allocate velocities (momentum terms)
    ...
    // Initialize plasticity masks to 1.0 (fully plastic initially)
    for (int i = 0; i < input_size * hidden_size; i++) {
    ...
    // Initialize manifold attention weights
    float scale_key = sqrtf(2.0f / (hidden_size + manifold_size));
    for (int i = 0; i < hidden_size * manifold_size; i++) {
        net->Key_weights[i] = randn() * scale_key;
        net->Value_weights[i] = randn() * scale_key;
        net->Query_weights[i] = randn() * scale_key;
    }
...
// ============================================================================
// FORWARD PASS WITH MOMENTUM-BASED WEIGHT EVOLUTION
// ============================================================================

EXPORT void ndm_forward(NDMNetwork *net, const float *input, float *output) {
    if (!net || !input || !output) return;
    
    const int I = net->input_size;
    const int H = net->hidden_size;
    const int O = net->output_size;
    const int M = net->manifold_size;
    
    // === Step 1: Compute input contribution ===
    float input_contrib[H];
    for (int j = 0; j < H; j++) {
        input_contrib[j] = 0.0f;
        for (int i = 0; i < I; i++) {
            input_contrib[j] += net->W_input[i * H + j] * input[i];
        }
    }
    ...
    for (int j = 0; j < H; j++) {
        recurrent_contrib[j] = 0.0f;
        for (int i = 0; i < H; i++) {
            recurrent_contrib[j] += net->W_hidden[i * H + j] * net->hidden_state[i];
        }
    }
    
    // === Step 3: Memory manifold read ===
    float queries[M];
    for (int m = 0; m < M; m++) {
        queries[m] = 0.0f;
        for (int i = 0; i < H; i++) {
    ...
    // === Step 4: Update hidden state using ODE ===
    for (int i = 0; i < H; i++) {
    ...
    // === Step 6: Compute weight derivatives (neuroplasticity) ===
    ...
    // === Step 7: Integrate weight ODEs WITH MOMENTUM ===
    // V(t+1) = β * V(t) + α * dW/dt
    // W(t+1) = W(t) + V(t+1)
    
    for (int i = 0; i < I * H; i++) {
        net->V_input[i] = net->momentum * net->V_input[i] + 
                          net->dt * net->base_plasticity * net->dW_input_dt[i];
        net->W_input[i] += net->V_input[i];
    }
    
    for (int i = 0; i < H * H; i++) {
        net->V_hidden[i] = net->momentum * net->V_hidden[i] + 
                           net->dt * net->base_plasticity * net->dW_hidden_dt[i];
        net->W_hidden[i] += net->V_hidden[i];
    }
    ...
    for (int i = 0; i < I * H; i++) {
        weight_velocity += fabsf(net->V_input[i]);
    }
    for (int i = 0; i < H * H; i++) {
        weight_velocity += fabsf(net->V_hidden[i]);
    }
...
EXPORT float ndm_train(NDMNetwork *net, const float *input, const float *target) {
    ...
    // Boost plasticity on error (adaptive learning)
...
EXPORT float ndm_get_momentum(NDMNetwork *net) {
    return net ? net->momentum : 0.0f;
}

EXPORT void ndm_set_momentum(NDMNetwork *net, float momentum) {
    if (net) {
        net->momentum = clip_value(momentum, 0.0f, 0.999f);
    }
}
...
EXPORT void ndm_print_info(NDMNetwork *net) {
    ...
    printf("╔════════════════════════════════════════════════════════════════╗\n");
    printf("║   NEURAL DIFFERENTIAL MANIFOLDS WITH MOMENTUM (NDM-M)        ║\n");
    printf("║   Momentum-Damped Continuous Weight Evolution                ║\n");
    printf("╚════════════════════════════════════════════════════════════════╝\n\n");
    ...
    printf("  Momentum Coefficient:  %.4f (β)\n", net->momentum);
