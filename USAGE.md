...
This API provides a simple interface to two powerful memory-native neural network models:
...
1. **Compile the C libraries** (required for both models):
...
```python
from api import AMRC, PMRC, create_model
```
...
# Create either model type dynamically
...
### Common Methods (Both AMRC and PMRC)

#### `fit(X, y, epochs=100, batch_size=None, verbose=0, reset_memory=False)`
...
- `reset_memory` (bool): Reset memory before training
...
- `method` (str): 'custom' (AMRC), 'custom'/'joblib'/'pickle' (PMRC)
...
| Parameter | Range | Description | When to Use |
|-----------|-------|-------------|-------------|
| `beta` | 0.0-1.0 | Memory preservation strength | Higher = stronger retention of past outputs |
| `alpha` | 0.0-1.0 | Memory update rate | Higher = faster memory updates |
| `output_beta` | 0.0-1.0 | Output layer memory (PMRC) | Enable output-level memory preservation |

### Architecture Flags (PMRC only)

| Flag | Default | Description |
|------|---------|-------------|
| `use_recurrent` | True | Enable recurrent connections |
| `use_learnable_gates` | False | Let network learn memory dynamics |
| `use_output_memory` | False | Enable memory in output layer |
...
### Pattern 5: Partial Training by Magnitude
...
| Advanced partial training | ❌ | ✅ |
| Serialization formats | Binary | Binary/Joblib/Pickle |
| Performance | Fastest | Very Fast |
| Best for | Production, Speed | Research, Flexibility |

## 📊 Performance Tips

1. **Start with AMRC** for simpler tasks and faster training
2. **Use PMRC** when you need the network to learn memory dynamics
3. **Set beta=0** to disable memory and get a standard feedforward network
4. **Enable learnable gates** when patterns are complex and you're unsure what to remember
5. **Use partial training** to prevent catastrophic forgetting in continual learning scenarios
6. **Save models frequently** during long training runs
...
3. **Frozen layers**: Remember to unfreeze when done with partial training
   ```python
...
- **Solution**: Check beta value (beta=0 means no memory)

**Problem**: Training is slow
- **Solution**: Use AMRC instead of PMRC, or reduce hidden_size

**Problem**: Model forgets previous knowledge
- **Solution**: Use partial training (freeze layers) or don't reset memory
...
- **Benchmarks**: Compare AMRC vs PMRC on your specific task
